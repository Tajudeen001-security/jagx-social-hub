...
import { Heart, MessageCircle, Share2, Music, Eye, Gift, Send, UserPlus, X } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div ref={containerRef} onScroll={handleScroll} className="h-screen snap-y snap-mandatory overflow-y-auto">
        {reels.map((reel, i) => (
          <ReelItem key={reel.id} reel={reel} isActive={i === currentIndex} user={user} navigate={navigate} />
        ))}
        {reels.length === 0 && (
...
const ReelItem = ({ reel, isActive, user, navigate }: { reel: Reel; isActive: boolean; user: any; navigate: any }) => {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from("likes").select("id").eq("post_id", reel.id).eq("user_id", user.id).single()
  ...
  const handleLike = async () => {
    ...
    if (liked) {
      ...
      if (reel.user_id !== user.id) {
        await supabase.from("notifications").insert({ user_id: reel.user_id, from_user_id: user.id, type: "like", content: "liked your reel", related_post_id: reel.id });
      }
    }
  };
  ...
  const sendGift = async () => {
    ...
    const { error } = await supabase.from("gifts").insert({
      sender_id: user.id, recipient_id: reel.user_id, post_id: reel.id, coin_amount: giftAmount, gift_type: "reel",
    });
  ...
  const followUser = async () => {
    ...
    if (error && error.code !== "23505") toast.error("Failed to follow");
    else toast.success(`Following @${reel.username}`);
  };
  ...
  return (
    <div className="relative h-screen snap-start bg-black">
      <video ref={videoRef} src={reel.video_url} className="absolute inset-0 w-full h-full object-cover"
        loop muted playsInline preload="metadata" onDoubleClick={handleLike} />
      ...
      <div className="absolute right-3 bottom-28 flex flex-col items-center gap-5">
        ...
        <button onClick={() => setShowGift(!showGift)} className="flex flex-col items-center gap-1">
          ...
          <span className="text-[10px] font-semibold text-gold">Gift</span>
        </button>
          ...
          <motion.div initial={{ y: 100 }} animate={{ y: 0 }} exit={{ y: 100 }} className="absolute bottom-20 left-3 right-3 p-3 rounded-xl bg-black/80 backdrop-blur-xl border border-gold/20">
            <div className="flex items-center gap-2 flex-wrap">
              {[10, 50, 100, 500, 1000].map(amt => (
                <button key={amt} onClick={() => setGiftAmount(amt)} className={`px-3 py-1.5 rounded-lg text-xs font-bold ${giftAmount === amt ? "gold-gradient text-primary-foreground" : "bg-white/10 text-white"}`}>
                  🪙 {amt}
                </button>
              ))}
      ...
      <AnimatePresence>
        {showComments && (
          <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }}
            className="absolute inset-0 bg-background/95 backdrop-blur-xl z-50 flex flex-col">
